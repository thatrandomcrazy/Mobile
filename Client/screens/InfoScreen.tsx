      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <Card colors={colors}>
          ...
          <Text style={[s.subtle, { marginTop: 2 }]}>
            Near Habima • Parking on site
          </Text>

          {/* MapView אמיתי */}
          <View style={[s.mapWrap, { borderColor: colors.border, backgroundColor: colors.card }]}>
            <MapView
              style={s.map}
              initialRegion={{
                latitude: LAT,
          ...
          {[
            ...
            <View key={i} style={[s.rowBetween, s.rowPad, i > 0 && s.sep]}>
              ...
              <Text
                style={[
                  s.text,
                  h === "Closed" ? { color: colors.danger, fontWeight: "700" } : null,
                ]}
              >
                {h}
              </Text>
...
function onMapPress(e: MapPressEvent) {
  // אפשר לפתוח ניווט גם בלחיצה על המפה
  openDirections();
}
...
function SectionHeader({
  ...
  return (
    <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
      ...
      <Text style={{ marginLeft: 8, fontSize: 16, fontWeight: "800", color: colors.text }}>
        {title}
      </Text>
    </View>
  );
