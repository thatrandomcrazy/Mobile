...
export const register = async (req: Request, res: Response) => {
  ...
  if (!name || !password || !phone) return res.status(400).json({ message: "name, password and phone required" });
  try {
    ...
    const user = await User.create({ username: name, password: hashedPassword, phone: formattedPhone, phoneVerified: false });
    const token = signToken(user);
    res.status(201).json({ message: "user registered successfully", data: { name: user.username, role: user.role }, token });
  } catch (error) {
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = signToken(user);
    res.status(200).json({ message: "logged in successfully", data: { name: user.username, role: user.role }, token });
  } catch (error) {
    res.status(500).json({ message: "server error", error: (error as Error).message });
  }
};
...
export const verifyOtpLogin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (check.status !== "approved") return res.status(400).json({ message: "Invalid code" });
    ...
    const token = signToken(user);
    res.status(200).json({ message: "login by phone success", token, data: { name: user.username, role: user.role } });
  } catch (error) {
...
export const verifyOtpRegister = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existsByUsername) return res.status(400).json({ message: "username already taken" });
    const existsByPhone = await User.findOne({ phone });
    if (existsByPhone) return res.status(400).json({ message: "phone already registered" });
    const salt = await bcrypt.genSalt(10);
    const hashed = await bcrypt.hash(password || "phone_only_auth", salt);
    const user = await User.create({ username, password: hashed, phone, phoneVerified: true });
    const token = signToken(user);
    res.status(201).json({ message: "registered by phone success", data: { name: user.username, role: user.role }, token });
  } catch (error) {
